...
def generate_note_for_date(client, target_date):
    weekday = target_date.weekday()
    topic_label, topic_instruction = TOPICS[weekday]
    date_str = target_date.strftime("%B %-d, %Y")

    user_prompt = f"""Today is {date_str} ({target_date.strftime('%A')}).
...
Remember: 500-600 words, prose only, end with the one thing to try."""
    ...
    return message.content[0].text, topic_label, date_str
...
def save_note_html(content, topic_label, date_str, target_date):
    os.makedirs("notes", exist_ok=True)
    slug = topic_label.lower().replace(" ", "-").replace("&", "and")
    filename = f"notes/{target_date.strftime('%Y-%m-%d')}-{slug}.html"
...
if __name__ == "__main__":
    ...
    # Generate for last 7 days (excluding today — handled by daily workflow)
    dates_to_generate = [today - datetime.timedelta(days=i) for i in range(1, 8)]

    print(f"📚 Backfilling {len(dates_to_generate)} days of notes...\n")

    for target_date in dates_to_generate:
        slug = TOPICS[target_date.weekday()][0].lower().replace(" ", "-").replace("&", "and")
        expected_file = f"notes/{target_date.strftime('%Y-%m-%d')}-{slug}.html"

        if os.path.exists(expected_file):
            print(f"⏭️  {target_date} — already exists, skipping")
            continue

        print(f"⏳ Generating: {target_date.strftime('%A, %B %-d, %Y')}...")
        content, topic_label, date_str = generate_note_for_date(client, target_date)
        save_note_html(content, topic_label, date_str, target_date)

        # Small delay to be kind to the API
        time.sleep(2)

    update_index()
    print("\n🎉 Backfill complete! All notes generated and index updated.")
